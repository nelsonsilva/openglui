...
loadShader(final type, final program) {
  ...
  if (gl.getShaderParameter(shader, RenderingContext.COMPILE_STATUS) != true) {
...
void initShaders() {
  var vertexShader = loadShader(RenderingContext.VERTEX_SHADER,
      VERTEX_PROGRAM);
  var fragmentShader = loadShader(RenderingContext.FRAGMENT_SHADER,
      FRAGMENT_PROGRAM);
  ...
  if (gl.getProgramParameter(shaderProgram, RenderingContext.LINK_STATUS) != true) {
...
void initBuffers() {
  ...
  gl.bindBuffer(RenderingContext.ARRAY_BUFFER, plotPositionBuffer);
  gl.vertexAttribPointer(aPlotPosition, 3, RenderingContext.FLOAT,
      false, 0, 0);
}
